...
const Hero = () => {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start start", "end start"], // triggers while section leaves
  });

  // Move and fade hero content as you scroll
  const y = useTransform(scrollYProgress, [0, 1], ["0%", "-80%"]);
  const opacity = useTransform(scrollYProgress, [0, 0.8], [1, 0]);

  return (
    <section ref={ref} className="relative h-[200vh] bg-black">
      {/* This makes hero last two scroll heights */}
      <motion.div
      ...
      >
        ...
        <motion.div
          style={{ opacity, y }}
          className="z-20 relative transition-all duration-700"
        >
          <Navbar />
        </motion.div>
        ...
        <motion.div
          style={{ y, opacity }}
          className="relative z-10 flex flex-col justify-center h-full px-6 md:px-16 max-w-3xl"
        >
